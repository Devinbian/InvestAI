...
*以上分析基于公开市场数据，仅供参考，投资有风险，决策需谨慎*`,
      ...
      response = {
        id: generateMessageId(),
        role: "assistant",
        content: `基于您的投资偏好和当前市场环境，我为您推荐以下优质股票：`,
        hasStockInfo: true,
...
📊 **个性化提醒**：建议您定期查看资产配置情况，根据市场变化及时调整投资策略。`,
      ...
      response = {
        id: generateMessageId(),
        role: "assistant",
        content: `# ${stockInfo.name}(${stockInfo.code}) 交易信息
...
### 投资要点
- **风险控制**：根据自己的风险承受能力确定购买数量
- **分散投资**：不要把所有资金投入单一股票
- **动态调整**：关注市场动态，适时调整投资策略
...
## 实施建议
1. **风险控制**：严格设置止损位，控制单笔损失
2. **仓位管理**：分散投资，避免集中持仓
3. **回测验证**：在实盘前进行充分的历史数据回测
4. **持续优化**：根据市场变化调整策略参数

💡 **温馨提示**：可以在侧边栏的"量化分析"中进行详细的策略回测和因子分析。`,
...
## 策略评价
该策略在历史数据中表现良好，具有较好的风险调整收益。

⚠️ **重要提示**：历史表现不代表未来收益，投资需谨慎。`,
      };
...
- 价格跌破均值-2σ时买入
- 成交量放大至少150%
- MACD金叉且RSI(14)>50且<70

#####2.建仓计划
- 首次建仓：33.6-33.8元区间，买入**30%**仓位
- 回调加仓：若价格回调至33.3元（20日均线上方），追加**20%**仓位
...
4. 结合宏观环境优化模型`;
      ...
      response = {
        id: generateMessageId(),
        role: "assistant",
        content: `# ${stockInfo.name}(${stockInfo.code}) 股票分析
...
### 基本面分析
${stockInfo.analysis.fundamental}

### 技术面分析
${stockInfo.analysis.technical}

### 投资建议
${stockInfo.analysis.recommendation}

### 风险提示
${stockInfo.analysis.risk}`,
...
| 股票代码 | 股票名称 | 当前价格 | 涨跌幅 |
|---------|---------|---------|--------|
| 000001  | 平安银行 | 12.45   | +1.88% |
| 600519  | 贵州茅台 | 1678.90 | -0.74% |
| 300750  | 宁德时代 | 185.50  | +4.62% |
...
这样的markdown格式展示了markdown-it的强大解析能力！

## 问题重现测试

### 多个#符号测试
####股票【600584】长电科技-智能量化交易策略
####一、策略定位
